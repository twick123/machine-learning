...
class SVMStruct:
    def __init__(self, dataSet, labels, C, toler, kernelOption, b):
...
def selectAlpha_j(svm, alpha_i, error_i):
    ...
    candidateAlphaList = nonzero(svm.errorCache[:, 0].A)[0]  # mat.Areturn array
...
def trainSVM(train_x, train_y, C, toler, maxIter, b, kernelOption = ('rbf', 1.0)):
    # calculate training time
    startTime = time.time()

    # init data struct for svm
    svm = SVMStruct(mat(train_x), mat(train_y), C, toler, kernelOption, b)
...
dataSet = []
labels = []
fileIn = open('F:\Python\dataset/testSet.txt')
...
maxIter = 50
b = 0
svmClassifier = trainSVM(train_x, train_y, C, toler, maxIter, b, kernelOption = ('linear', 0))
